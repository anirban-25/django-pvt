...
def dg(param):
    if param["has_dangerous_item"]:
        logger.info(f'[SURCHARGE] DXT: dg')
        return {
            "name": "Dangerous Goods",
            "description": "",
            "value": param["quote_obj"].fee * 0.2,
        }
    else:
        return None
...
def hd(param):
    if (
        ...
        logger.info(f'[SURCHARGE] DXT: hd {param["pu_address_type"]}, {param["de_to_address_type"]}')
        return {
...
def ol(param):
    logger.info(f'[SURCHARGE] DXT: ol {param["max_dimension"]}')
...
def tgp_tgd(param):
    if param["is_tail_lift"]:
        logger.info(f'[SURCHARGE] DXT: tgp_tgd {param["is_tail_lift"]}')
        return {
            "name": "Tailgate",
