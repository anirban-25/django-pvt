...
def timedelta_2_hours(timedelta):
...
def get_eta_with_suburb(suburb, unit="days"):
    """
    This function can be used to decide the ETA when booking has no quote
    """

    if not suburb:
        return 5 if unit == "days" else 5 * 25
    else:
        if suburb.upper() in ["WA", "TAS"]:
            return 5 if unit == "days" else 5 * 24
        else:
            return 3 if unit == "days" else 3 * 24
